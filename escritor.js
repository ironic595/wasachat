async function editarYEnviarHolograma({slots, codeId, email, nombre, accessCode, tx_hash, percent}) {
  ...
  const fontSize=Math.floor(W*0.038); const yPos=H-Math.floor(H*0.065);
  ...
  ctx.fillStyle='#E8F0FF'; ctx.strokeStyle='rgba(0,0,0,0.6)'; ctx.lineWidth=Math.max(1,fontSize*0.08);
